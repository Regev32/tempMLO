...
def run_original_grim(path_configuration,hap_pop_pair=True,Producehpf=False,dominant3=True):
    ...
    #changing donor file to 3 most important gls and returning short_gl,extra_gl for each row in donor
    if dominant3:
        path_donor = config["imputation_in_file"]

        gls , lines = change_donor_file(path_donor) #change so won't change donor file

    #imputation
    graph_path = os.path.join(
        config.get("imputation_out_path", "output"),
        config.get("graph_for_later", "graph.pkl")
    )
    try:
        with open(graph_path, 'rb') as f:
            graph = pkl.load(f)
        print("graph loaded!!")
    except (FileNotFoundError, EOFError, pkl.UnpicklingError):
        graph = None
        print("graph = None")

    graph = impute(conf_file=path_configuration,hap_pop_pair= hap_pop_pair, graph=graph)
    with open(graph_path, "wb") as out:
        pkl.dump(graph, out)
    print(f"Saved graph pickle → {graph_path}")
...
if __name__ == "__main__":
    ...
    run_original_grim(path_configuration=conf_file,hap_pop_pair=True,Producehpf=False,dominant3=True)
